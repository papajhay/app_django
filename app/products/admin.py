...
#    class Meta:
#        model = Product
#        fields = ('name', 'description_de', 'description_en', 'description_fr', 'price', 'image')


## Héritage corrigé : ImportExportModelAdmin en premier
#class BaseProductAdmin(
#    ImportExportModelAdmin,     # ← En premier pour l'import/export
#    TabbedTranslationAdmin,
#    UnfoldAdmin,
#):
#    pass
...
#@admin.register(Product)
#class ProductAdmin(BaseProductAdmin):
#    resource_class = ProductResource  # ← Sans liste [] ! Juste la classe

#    list_display = ('id', 'name', 'short_description', 'price', 'image_preview', 'created_at')
#    list_display_links = ('name',)
#    list_filter = ('created_at', 'updated_at')
#    search_fields = ('name', 'description')
#    ordering = ('-created_at',)
...
#    def make_discount(self, request, queryset):
#        updated = 0
#        logger.info(f"Début action réduction 10% par {request.user} – {queryset.count()} produits")
#        for product in queryset:
#            old_price = product.price
#            product.price = (product.price * Decimal('0.9')).quantize(Decimal('0.01'))
#            product.save(update_fields=["price", "updated_at"])
...
#        logger.info(f"Réduction 10% appliquée sur {updated} produit(s) par {request.user}")
#        if updated > 10:
#            logger.warning("Volume important : %d produits modifiés", updated)
#        self.message_user(request, _(f"Réduction de 10 % appliquée sur {updated} produit(s)."))
...
#        super().save_model(request, obj, form, change)
#        if change:
#            logger.info("Produit ID %s modifié par %s", obj.id, request.user)
#        else:
#            logger.info("Nouveau produit ID %s créé par %s", obj.id, request.user)
...
#    def image_preview(self, obj):
#        if obj.image:
#            return mark_safe(f'<img src="{obj.image.url}" width="80" style="border-radius: 8px; object-fit: cover;" />')
...
#            desc = getattr(obj, description_field)
#            if desc:
#                return strip_tags(desc)[:200] + ("..." if len(strip_tags(desc)) > 200 else "")
#        return strip_tags(obj.description)[:200] + ("..." if len(strip_tags(obj.description)) > 200 else "")

#    short_description.short_description = _("Description courte")